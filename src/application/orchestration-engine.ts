...
import type {
  ...
  ExecutionStep, RetryPolicy, RiskLevel, Run, Task, TaskClassification,
} from '../domain/models'
...
type AttemptFailure = { code: string; message: string; retryable: boolean; blocked: boolean }

const DEFAULT_RETRY: RetryPolicy = { maxAttempts: 1, backoffMs: 0, retryableErrors: ['TRANSIENT', 'TIMEOUT'] }
...
export class OrchestrationEngine {
  ...
  async createPlan(taskId: string, input: PlanInput, actor: Actor): Promise<ExecutionPlan> {
    ...
    const retryPolicy: RetryPolicy = {
      ...
      retryableErrors: Array.isArray(input.retryPolicy?.retryableErrors) ? [...input.retryPolicy.retryableErrors] : [...DEFAULT_RETRY.retryableErrors],
    }
  ...
  private classifyFailure(error: unknown, policy: RetryPolicy): AttemptFailure {
    if (error instanceof AdapterError) return {
      ...
      retryable: error.retryable && policy.retryableErrors.includes(error.code),
      blocked: error.kind === 'AUTHORIZATION' || error.kind === 'CREDENTIAL' || error.kind === 'UNSAFE_IDEMPOTENCY',
    }
    if (error instanceof DOMException && error.name === 'AbortError') return { code: 'TIMEOUT', message: 'Step execution timed out', retryable: policy.retryableErrors.includes('TIMEOUT'), blocked: false }
    return { code: 'UNEXPECTED', message: 'Adapter execution failed', retryable: false, blocked: false }
  }
  ...
  private async invokeWithTimeout(adapter: ExecutionAdapter, invocation: CapabilityInvocation, timeoutSeconds: number) {
    ...
    try {
      return await Promise.race([
        ...
        new Promise<never>((_, reject) => controller.signal.addEventListener('abort', () => reject(new DOMException('Execution timed out', 'AbortError')), { once: true })),
      ])
  ...
  async executePlan(planId: string, actor: Actor): Promise<{ plan: ExecutionPlan; results: ExecutionResult[] }> {
    ...
    for (const step of plan.steps) {
      ...
      if (Date.now() - orchestrationStarted >= plan.overallTimeoutSeconds * 1000) {
        await this.failStep(plan, task, step, actor, { code: 'TASK_TIMEOUT', message: 'Overall task timeout exceeded', retryable: false, blocked: false })
        return { plan, results }
      }
      ...
      if (!capability || capability.status !== 'ACTIVE') {
        await this.failStep(plan, task, step, actor, { code: 'CAPABILITY_UNAVAILABLE', message: 'Capability is not active', retryable: false, blocked: true })
        return { plan, results }
      }
      const adapter = this.adapters.resolve(capability.name)
      if (!adapter) {
        await this.failStep(plan, task, step, actor, { code: 'ADAPTER_UNAVAILABLE', message: 'No controlled adapter is registered', retryable: false, blocked: true })
        return { plan, results }
      }
      if (!adapter.supportsIdempotency && step.retryPolicy.maxAttempts > 1) {
        await this.failStep(plan, task, step, actor, { code: 'UNSAFE_IDEMPOTENCY', message: 'Adapter cannot safely retry side effects', retryable: false, blocked: true })
        return { plan, results }
      }

      const runId = newId('run')
      const idempotencyKey = `${plan.workspaceId}:${plan.id}:${step.id}`
      const timestamp = now()
      ...
      await this.audit('RUN_CREATED', actor, task.workspaceId, 'run', run.id, { taskId: task.id, stepId: step.id })
      ...
      for (let attempt = 1; attempt <= step.retryPolicy.maxAttempts; attempt++) {
        ...
        if (!refreshedTask || refreshedTask.status === 'CANCELLED') { run.status = 'CANCELLED'; step.status = 'CANCELLED'; plan.status = 'CANCELLED'; run.completedAt = now(); await this.repo.saveRun(run); await this.repo.savePlan(plan); return { plan, results } }
        ...
        try {
          ...
          await this.audit('EXECUTION_STARTED', actor, task.workspaceId, 'run', runId, { planId: plan.id, stepId: step.id, capabilityId: capability.id, attempt, adapterId: adapter.id })
          const adapterResult = await this.invokeWithTimeout(adapter, invocation, step.timeoutSeconds)
          ...
          await this.repo.saveRun(run); await this.repo.savePlan(plan)
          await this.audit('EXECUTION_COMPLETED', actor, task.workspaceId, 'run', runId, { planId: plan.id, stepId: step.id, capabilityId: capability.id, attempt })
          ...
          await this.audit('EXECUTION_FAILED', actor, task.workspaceId, 'run', runId, { planId: plan.id, stepId: step.id, capabilityId: capability.id, attempt, errorCode: failure.code, retryable: failure.retryable })
          if (failure.retryable && attempt < step.retryPolicy.maxAttempts) {
            ...
            await this.audit('RETRY_SCHEDULED', actor, task.workspaceId, 'execution_step', step.id, { runId, attempt, nextAttempt: attempt + 1, backoffMs: step.retryPolicy.backoffMs })
    ...
    await this.audit('TASK_COMPLETED', actor, task.workspaceId, 'task', task.id, { planId: plan.id, resultCount: results.length })
    return { plan, results }
  }
  ...
  private async failStep(plan: ExecutionPlan, task: Task, step: ExecutionStep, actor: Actor, failure: AttemptFailure, run?: Run) {
    ...
    if (run) { run.status = failure.blocked ? 'BLOCKED' : 'FAILED'; run.error = failure.message; run.completedAt = timestamp; run.updatedAt = timestamp; await this.repo.saveRun(run) }
  ...
  async cancel(taskId: string, actor: Actor) {
    ...
    for (const run of runs) { run.status = 'CANCELLED'; run.completedAt = now(); run.updatedAt = run.completedAt; await this.repo.saveRun(run) }
    await this.audit('TASK_CANCELLED', actor, task.workspaceId, 'task', task.id, { planId: plan?.id ?? null })
    return task
  }
