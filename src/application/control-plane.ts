...
export class ControlPlane {
  ...
  async createCapability(input: ObjectInput, actor: Actor) {
    ...
    const timestamp = now(); const value: Capability = { id, name: text(input.name, 'name'), description: text(input.description, 'description', false) ?? '', ownerScope, workspaceId, riskLevel: oneOf(input.riskLevel, ['LOW','MEDIUM','HIGH','CRITICAL'] as const, 'riskLevel'), inputSchema: object(input.inputSchema, 'inputSchema'), outputSchema: object(input.outputSchema, 'outputSchema'), status: oneOf(input.status, CAPABILITY_STATUSES, 'status', 'ACTIVE'), createdAt: timestamp, updatedAt: timestamp }
  ...
  async updateCapability(id: string, input: ObjectInput, actor: Actor) { assertNoPlaintextSecrets(input); const value = await this.repo.getCapability(id); if (!value) throw notFound('Capability'); await this.ownerOnly(actor, value.workspaceId, `capability:${id}`, 'update'); if (input.name !== undefined) value.name = text(input.name, 'name'); if (input.description !== undefined) value.description = text(input.description, 'description'); if (input.riskLevel !== undefined) value.riskLevel = oneOf(input.riskLevel, ['LOW','MEDIUM','HIGH','CRITICAL'] as const, 'riskLevel'); if (input.inputSchema !== undefined) value.inputSchema = object(input.inputSchema, 'inputSchema'); if (input.outputSchema !== undefined) value.outputSchema = object(input.outputSchema, 'outputSchema'); if (input.status !== undefined) value.status = oneOf(input.status, CAPABILITY_STATUSES, 'status'); value.updatedAt = now(); await this.repo.saveCapability(value); await this.audit('CAPABILITY_UPDATED', actor, value.workspaceId, 'capability', id, { fields: Object.keys(input), outcome: 'SUCCESS' }); return value }
  ...
  async createRun(input: ObjectInput, actor: Actor) {
    ...
    const timestamp = now(); const value: Run = { id, taskId, workspaceId: task.workspaceId, status: 'CREATED', attempt: 0, idempotencyKey: null, startedAt: null, completedAt: null, error: null, createdAt: timestamp, updatedAt: timestamp }; await this.repo.createRun(value); await this.audit('RUN_CREATED', actor, value.workspaceId, 'run', id, { taskId, outcome: 'SUCCESS' }); return value
  }
